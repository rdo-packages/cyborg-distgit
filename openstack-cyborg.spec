...
PYTHONPATH=. oslo-config-generator --config-file=tools/config/%{service}-config-generator.conf --output-file etc/%{service}/%{service}.conf
# Generate a sample policy.yaml file for documentation purposes only
PYTHONPATH=. oslopolicy-sample-generator --config-file=tools/config/%{service}-policy-generator.conf --output-file etc/%{service}/policy.yaml
%{py3_build}
...
install -p -D -m 640 etc/%{service}/%{service}.conf %{buildroot}%{_sysconfdir}/%{service}/%{service}.conf
...
rm -f %{service}/tests/unit/test_hacking.py
PYTHON=%{__python3} stestr run --slowest --concurrency 2
...
%config(noreplace) %attr(0640, %{service}, %{service}) %{_sysconfdir}/%{service}/policy.yaml
